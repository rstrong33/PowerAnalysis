function power_results = PowerAnalysis(prefs)

if ischar(prefs.data)
    if ~isempty(strfind(prefs.data, '.xls'))
        prefs.data = xlsread(prefs.data);
    elseif ~isempty(strfind(prefs.data, '.csv'))
        prefs.data = csvread(prefs.data);
    end
end
